...
# Parameters
N=128 # Batchsize
log_interval =150 # ploting frequence
num_epoch=3 # Num epoch
...
try:
    ...
    print ("Directory creation %s failed" % args_dico['save_dir'])
else:
...
folders_notMNIST = os.listdir('./MNIST_TASK/notMNIST/')

NotMNIST_x_list = []
NotMNIST_y_list_onehot = []
NotMNIST_y_list = []
for idx, folder in enumerate(folders_notMNIST):
    files_notMNIST = os.listdir('./MNIST_TASK/notMNIST/' + folder)

    for file in files_notMNIST:
        img_NotMNIST = cv2.imread('./MNIST_TASK/notMNIST/' + folder + '/' + file, 0)
...
trainloader_NOTmnist = torch.utils.data.DataLoader(DatasetFromFolder(NotMNIST_x, NotMNIST_y,transform=transform, phase='test'), batch_size=N, shuffle=False, num_workers=0)



net = ThreehiddedLayersNet_fixed(D_in, H1,H2,H3,nb_class,0.1)

#estimation of mean outputs
...
print("net_mu and net_variance:" , net_mu,net_var)
...
for step in range(nb_models):
    ...
    utils_functions.bn_update(trainloader_mnist, net4, 'cpu')
...
BS = ((target_onehot.type(torch.FloatTensor) - output_proba) * 
      (target_onehot.type(torch.FloatTensor) - output_proba)).sum().item()
